...
function Sidebarr(props) {
  ...
  const [collapsed, setCollapsed] = React.useState(false); // Set initial collapse state based on screen size
      ...
      >
        <div className="flex items-center justify-between">
          <button className="md:hidden" onClick={() => { setCollapsed(!collapsed); }}>
            <img src={menu} className="h-6 ml-5 md:hidden" />
          </button>
          <SidebarHeader className="md:float-right"/>
        </div>
        <div style={{ flex: 1, marginBottom: "2px" }}>
          ...
          >
            ...
            <MenuItem onClick={() => { openSignOutPopup() }} icon={<Box component="img" src={logout} alt={logout} className="h-7" />}> Sign Out </MenuItem>
          </Menu>
        </div>
      </Sidebar>
    </div>
